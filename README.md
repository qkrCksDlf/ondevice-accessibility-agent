...
<img src="images/kotlin.svg" alt="Kotlin · Jetpack Compose" height="20"> <img src="images/llamacpp.svg" alt="llama.cpp · C++ JNI" height="20"> <img src="images/langgraph.svg" alt="LangGraph · ReAct" height="20"> <img src="images/mcp.svg" alt="MCP · Tools" height="20">
...
### 평가 하네스의 발전: 비결정성 발견 → 120케이스 확장

120케이스 하네스에 앞서, 클라우드 모드의 도구 라우팅을 **1차 30케이스 하네스**(Gemini, temperature 0.2)로 먼저 검증했습니다. 이때 두 실행이 같은 점수(83.3%)를 기록했지만 실패한 케이스가 서로 다르다는 것을 발견해, 단일 실행 결과가 비결정적임을 확인했습니다. 반복 측정으로 항상 재현되는 체계적 실패와 실행마다 뒤집히는 플레이키(flaky) 실패를 분리한 뒤, 체계적 실패만 시스템 프롬프트 재작성(도구 사용 강제·응답 구조화 가이드)으로 수정해 라우팅 정확도를 **83.3%(25/30)에서 96.7%(29/30)**로 개선했습니다. 다만 1차 하네스는 복합 요청 시나리오가 3케이스에 불과해 커버리지를 과대평가할 수 있었고, 이를 보완하기 위해 위 표의 120케이스 하네스로 확장하면서 복합 요청의 실제 개선 여지가 드러났습니다.
